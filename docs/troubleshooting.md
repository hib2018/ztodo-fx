...
- invalid envelope/Proposal: fxとztodo-fxのversionを確認し、Proposalを再生成します。無効出力は保存されません。
...
`ztodo-fx doctor`はモデル要求を送らず、pathと外部CLIの状態を表示します。prompt、Issue本文、fx response、credentialは通常診断へ出しません。
