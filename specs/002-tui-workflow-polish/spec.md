...
5. **Given** Issue一覧を繰り返し更新する、**When** 応答に重複・不正UTF-8・順序変化がある、**Then** 不正な更新は既存一覧を壊さず、正常な一覧は一意かつ決定的な順序で表示される。
...
- **FR-015**: Issue snapshotはRepositoryとIssue番号で一意化し、Repository名・Issue番号順へ正規化し、不正UTF-8または応答内重複を状態変更前に拒否しなければならない。
- **FR-016**: Issueメニューは登録RepositoryへのIssue作成、選択Issueのタイトル・本文編集、Close・Reopenを確認付きで実行し、成功後にGitHubから再同期しなければならない。
- **FR-017**: GitHub側の変更後に再同期またはローカル保存が失敗した場合、リモート変更済みであることと再同期方法を明示しなければならない。
